...
$M_X$ & $\Lambda_*Q_{\kker}^{-14/3}\Xi_0^{2/3}$, $\Xi_0=1.22965$ & $3.4638\times 10^{15}\,\mathrm{GeV}$\\
$\tau_p/\tau_p^{\mathrm{HK,local}}$ & Proton lifetime ratio & $1.1649$\\
...
\noindent\textbf{GUT scale formula.}  The GUT-scale gauge boson mass follows the determinant master law:
\begin{equation}
M_X(\tau) = M_C(\tau)\,R_{\rm trip}(\tau)^{2/9}
= \Lambda_*\,Q_{\kker}^{-14/3}\,\Xi_0(\tau)^{2/3},
\end{equation}
where $R_{\rm trip}:=d_{10}d_{01}d_{11}/d_0^3$ is a modular scalar from the bridge determinant data, and $\Xi_0(\tau_{\kker})=1.22965$ at the strict kernel.  This gives $M_X=3.4638\times10^{15}\,\mathrm{GeV}$ and $\tau_p/\tau_p^{\rm HK,local}=1.1649$.

The split-ratio approximation $\Xi_0\approx\sqrt{r/s}=\sqrt{3/2}\approx1.2247$ (for the $(3,2)$-split) agrees to within $0.04\%$, but the exact value requires the determinant master law above.
