...
\section{Adiabatic Connection}\label{sec:adiabatic}

\begin{theorem}[Adiabatic-connection form]\label{thm:adiabatic}
Suppose the full energy has a \emph{linear bridge coupling}
parameterised by $\lambda\in[0,1]$:
\begin{equation}\label{eq:linearcoupling}
...
\label{eq:adiabatic2}
\end{align}
where $\mathbb{E}_{\lambda,X}$ and $\Cov_{\lambda,X}$ are with
respect to the $\lambda$-dependent conditional Gibbs measure.

By Taylor's theorem with integral remainder:
\begin{equation}\label{eq:adiabaticfull}
\boxed{\Gamma_{\beta,1}(X)
= \Gamma_{\beta,0}(X)
+ \partial_\lambda\Gamma_{\beta,\lambda}(X)\big|_{\lambda=0}
...
\begin{proof}
\emph{Step 1: First $\lambda$-derivative.}

Differentiating $\Gamma_{\beta,\lambda}$ with respect to $\lambda$:
\begin{align}
\partial_\lambda\Gamma_{\beta,\lambda}(X)
&= -\beta^{-1}\frac{\int(-\beta)(-\langle X,B(Z)\rangle)
e^{-\beta\Phi_\lambda}\,dZ}
{\int e^{-\beta\Phi_\lambda}\,dZ}\\
&= -\frac{\int\langle X,B(Z)\rangle\,e^{-\beta\Phi_\lambda}\,dZ}
{\int e^{-\beta\Phi_\lambda}\,dZ}\\
&= -\langle X,\mathbb{E}_{\lambda,X}[B]\rangle.
\end{align}

\emph{Step 2: Second $\lambda$-derivative.}

Differentiating~\eqref{eq:adiabatic1} with respect to $\lambda$
using the same quotient-rule technique as in
Theorem~\ref{thm:hessian}:
\begin{align}
\partial_\lambda^2\Gamma_{\beta,\lambda}
&= -\langle X,\,\partial_\lambda\mathbb{E}_{\lambda,X}[B]\,\rangle.
\end{align}
The $\lambda$-derivative of the conditional expectation gives a
covariance (exactly as the $X$-derivative of $\mathbb{E}_X[\Phi_X]$
gave $\Cov_X(\Phi_X,\Phi_X)$ in Theorem~\ref{thm:hessian}):
\begin{equation}
\partial_\lambda\mathbb{E}_{\lambda,X}[B]
= \beta\,\Cov_{\lambda,X}(B,\langle X,B\rangle)
= \beta\,\Cov_{\lambda,X}(B,B)\,X.
\end{equation}
(Here we used the linearity of $\langle X,B\rangle$ in $B$.)
Therefore
$\partial_\lambda^2\Gamma_{\beta,\lambda}
=-\beta\langle X,\Cov_{\lambda,X}(B,B)X\rangle$.

\emph{Step 3: Taylor with integral remainder.}

\begin{equation}
\Gamma_{\beta,1}
= \Gamma_{\beta,0}
+ \partial_\lambda\Gamma_{\beta,\lambda}\big|_0
+ \int_0^1(1-\lambda)\,\partial_\lambda^2\Gamma_{\beta,\lambda}\,d\lambda.
\end{equation}
Substituting~\eqref{eq:adiabatic2}
gives~\eqref{eq:adiabaticfull}.
\end{proof}

\begin{remark}[DFT interpretation]
Equation~\eqref{eq:adiabaticfull} is the LoNalogy version of the
adiabatic connection formula in DFT.  The exchange-correlation
energy is the $\lambda$-integrated bridge covariance:
\begin{equation}
E_{xc}^{\LoN}[X] = -\int_0^1(1-\lambda)\,\beta\,
\langle X,\Cov_{\lambda,X}(B,B)\,X\rangle\,d\lambda.
\end{equation}
This is \emph{exact} and \emph{system-dependent}: the covariance
$\Cov_{\lambda,X}(B,B)$ varies with $\lambda$, $X$, and the
specific system.  It is not a universal fixed kernel.
\end{remark}
...
\section{Global Reaction-Path Patching}\label{sec:patching}

\begin{theorem}[Global reaction-path surrogate]\label{thm:patching}
Let $s\mapsto V(s)$ be a smooth 1D reaction-path potential with
finitely many non-degenerate stationary points
$s_1,\ldots,s_m$.  For each $s_j$, let $V_j^{\mathrm{mPT}}(x_j)$
be the local mPT normal form of
Theorem~\ref{thm:well}/\ref{thm:barrier}, with canonical
coordinate $x_j=x_j(s)$.  Define the local 4-jet:
\begin{equation}
J_j(x_j) := V(s_j) + \tfrac{1}{2}k_j x_j^2
+ \tfrac{1}{6}g_j x_j^3 + \tfrac{1}{24}h_j x_j^4.
\end{equation}
Let $\{\chi_j\}_{j=1}^m$ be a smooth partition of unity
subordinate to neighbourhoods $U_j$ of the stationary points,
and let $V_{\mathrm{reg}}(s)$ be a smooth interpolant on the
complement $[s_1,s_m]\setminus\bigcup_j U_j$.

Then the global surrogate
\begin{equation}\label{eq:globalLoN}
...
In the neighbourhood $U_j$ of $s_j$, $\chi_j(s_j)=1$ and
$\chi_k(s_j)=0$ for $k\neq j$.  Therefore
\begin{equation}
V_{\LoN}(s) = V_{\mathrm{reg}}(s)
+ V_j^{\mathrm{mPT}}(x_j(s)) - J_j(x_j(s))
\end{equation}
near $s_j$.  By construction $V_{\mathrm{reg}}$ is chosen so that
$V_{\mathrm{reg}}(s)=J_j(x_j(s))$ near $s_j$ (the 4-jet absorbs
the regular part).  Therefore
$V_{\LoN}(s)=V_j^{\mathrm{mPT}}(x_j(s))$ near $s_j$.

\emph{Step 2: Error bound.}

By Theorem~\ref{thm:well}/\ref{thm:barrier},
$V(s)-V_j^{\mathrm{mPT}}(x_j(s))=O(x_j^5)$.
Since $x_j(s)=s-s_j+O((s-s_j)^2)$ (the coordinate change has
unit linear coefficient), $x_j^5=O((s-s_j)^5)$.

\emph{Step 3: Global smoothness.}

The partition of unity $\{\chi_j\}$ ensures that $V_{\LoN}$
transitions smoothly between the mPT surrogates and the regular
interpolant.  The subtraction of $J_j$ prevents double-counting
of the polynomial part.
\end{proof}

\begin{remark}[Practical use]
For a reaction path with one reactant well, one transition state,
and one product well ($m=3$), the global surrogate consists of
three mPT patches (two wells and one barrier) glued by a partition
of unity.  Each patch is analytically solvable (Pöschl--Teller
bound states or transmission coefficients), providing closed-form
tunnelling rates and vibrational frequencies.
\end{remark}


%% ============================================================
\part{Numerical Benchmark: 4-Site Hubbard Model}
%% ============================================================

\section{Model and Setup}\label{sec:hubbard}

To validate the LoNalogy quantum chemistry framework beyond
abstract algebra, we benchmark on the half-filled 4-site
open-chain Hubbard model:
\begin{equation}\label{eq:hubbard}
H = -t\sum_{\langle ij\rangle,\sigma}
(c_{i\sigma}^\dagger c_{j\sigma}+\mathrm{h.c.})
+ U\sum_i n_{i\uparrow}n_{i\downarrow},
\end{equation}
with $t=1$ and $U/t\in\{2,4,8,12\}$, restricted to the $S_z=0$
sector (dimension 36).

\begin{definition}[Active space]\label{def:hubbardactive}
The active space $\mathcal{H}_A$ consists of the no-doublon
sector: all $S_z=0$ Fock states with zero double occupancy.
For the 4-site model, $\dim\mathcal{H}_A=6$.
\end{definition}

The canonical bridge projector
$P_B=\Pi_{\Ran(QHP_A)}$ yields:
\begin{equation}
\dim\mathcal{H}_A=6,\qquad
\dim\mathcal{H}_B=5,\qquad
\dim\mathcal{H}_I=25.
\end{equation}
The bridge rank 5 is smaller than $\dim\mathcal{H}_A=6$ and much
smaller than the full dimension 36, confirming genuine reduction.

\section{Verification of Three-Sector Form}\label{sec:verify}

\begin{proposition}[Numerical verification of canonical decoupling]\label{prop:numerical}
The transformed Hamiltonian satisfies
\begin{equation}
\|P_AHP_I\|_F = 9.5\times 10^{-16},
\end{equation}
confirming $P_AHP_I=0$ to machine precision
...
\begin{tabular}{rrrrr}
\toprule
$U/t$ & Exact & Bare active & LoN static gen & $|\mathrm{err}|$\\
\midrule
...
\begin{theorem}[Exact LoN root recovery]\label{thm:recovery}
The fixed-point iteration of the exact effective Hamiltonian
$H_A^{\eff}(E)$ reproduces the full exact ground-state energy to
machine precision ($<10^{-13}$) at all tested $U/t$ values.
\end{theorem}

\begin{proof}
Direct numerical computation.  The root-finding algorithm
(Brent's method) applied to
$f(E)=\lambda_0(H_A^{\eff}(E))-E$ converges to the exact
ground-state energy within floating-point tolerance.  The residual
$|f(E_*)|<10^{-13}$ at all tested coupling strengths.
\end{proof}

\begin{remark}[Strong-coupling improvement]
At $U/t=12$ (strong correlation / dissociation limit), the static
generalised LoN downfolding achieves $0.3\%$ error relative to the
exact ground-state energy, starting from a bare active energy of
exactly zero.  The improvement is $>200\times$ compared to bare
active.  This confirms that the canonical bridge captures the
essential correlation physics in the strong-coupling regime.

At $U/t=2$ (weak coupling), the $E_*=0$ reference energy is
suboptimal, and the static approximation underperforms.  This is
not a defect of the exact algebra (which recovers the full energy
exactly) but of the choice of linearisation point $E_*$.
Self-consistent $E_*$ selection or higher-order expansion would
improve the weak-coupling regime.
\end{remark}
...
\quad Adiabatic connection & Thm~\ref{thm:adiabatic} & Exact\\
...
\quad Global patching & Thm~\ref{thm:patching} & Exact\\
\midrule
\multicolumn{3}{l}{\textbf{(D) Numerical benchmark (4-site Hubbard)}}\\
\quad Exact LoN root recovery & Thm~\ref{thm:recovery} & Exact\\
\quad Canonical decoupling verified & Prop~\ref{prop:numerical} & Numerical\\
\bottomrule
\end{tabular}
\end{center}
...
\;+\;\text{Adiabatic bridge covariance}\;+\;\text{Patched mPT.}}
\end{equation}
